...
#%%
plt.figure(17)
plt.plot(range(1,16),accTest)
#plt.plot(accTest_median)
plt.plot(range(1,16),max_acc)
plt.plot(range(1,16),np.zeros(15)+13/23)

for sub in range(15):
    if perc_mean[sub]>97.5:
        plt.scatter(sub+1,accTest[sub],s=80,facecolors='k',edgecolors='b')
plt.legend(('Parameters based on validation sets','Optimum parameters','Baseline','p<0.025'))#'Parameters based on validation sets, median',
plt.scatter(range(1,16),accTest,s=80,facecolors='none',edgecolors='b')
plt.plot(range(1,16),max_acc,'kx')
